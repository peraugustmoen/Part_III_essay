...
# Simulation corresponding to Plot 8

ns = seq(200,550,by=50)
...
# Inflation factor of penalty term in Trimmed Lasso
lambda_inflate = 1.2



# Book keeping
sigma_hats_naive = matrix(data=NA, ncol = length(ns),nrow=N)
sigma_hats_dd = matrix(data=NA, ncol = length(ns),nrow=N)
sigma_hats_dd_fold = matrix(data=NA, ncol = length(ns),nrow=N)

v_1_hats_naive = matrix(data=NA, ncol = length(ns),nrow=N)
v_1_hats_dd = matrix(data=NA, ncol = length(ns),nrow=N)
v_1_hats_dd_fold = matrix(data=NA, ncol = length(ns),nrow=N)

beta_1_hats_naive = matrix(data=NA, ncol = length(ns),nrow=N)
beta_1_hats_dd = matrix(data=NA, ncol = length(ns),nrow=N)

v_2_hats_naive = matrix(data=NA, ncol = length(ns),nrow=N)
v_2_hats_dd = matrix(data=NA, ncol = length(ns),nrow=N)
v_2_hats_dd_fold = matrix(data=NA, ncol = length(ns),nrow=N)

beta_2_hats_naive = matrix(data=NA, ncol = length(ns),nrow=N)
beta_2_hats_dd = matrix(data=NA, ncol = length(ns),nrow=N)
...
for (l in 1:length(ns)) {
  ...
  for (r in 1:N) {
    ...
    # Standard/naive debias of the naive/standard Lasso
    ...
    # Inference on beta_1 with DD Lasso
    svd_1 = fast.svd(X[,-1])
    d1 = svd_1$d
    tau1 = median(d1)
    U1 = svd_1$u
...
filename = "simulation4_3.pdf"
setwd(directory)
pdf(file = filename, width = 10, height = 5)

par(mfrow=c(1,2),mar=c(4,4,4,4))

plot(y=coverage_1_naive, x=ns, ylab ="Coverage frequency", main = TeX("Coverage of $\\beta_1$. $\ p=600,q=30,\\sigma_{E}=1$"), type="l", ylim=c(0.1,1.0),xlab=TeX("$n$"))
lines(y=coverage_1_spectral,x= ns, col=2, type="l")
lines(y=coverage_1_spectral_fold,x= ns, col=3, type="l")
# Plotting grid 
for (i in 1:10) {
...
plot(y=coverage_2_naive, x=ns, ylab ="Coverage frequency", main = TeX("Coverage of $\\beta_{s+1}$. $\ p=600,q=30,\\sigma_{E}=1$"), type="l", ylim=c(0.1,1.0),xlab=TeX("$n$"))
lines(y=coverage_2_spectral,x= ns, col=2, type="l")
lines(y=coverage_2_spectral_fold,x= ns, col=3, type="l")
# Plotting grid
for (i in 1:10) {
  abline(h=i/10,col = "lightgray", lty = "dotted",
         lwd = par("lwd"))
...
mean(coverage_1_naive) #0.3676875
mean(coverage_1_spectral) #0.923
mean(coverage_1_spectral_fold) #0.93675

mean(coverage_2_naive) #0.9256875
mean(coverage_2_spectral) #0.931875
mean(coverage_2_spectral_fold) #0.943
